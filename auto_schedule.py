...
# ==========================================
# 2. ロジック関数群（generate_schedule.py からの完全移植）
# ==========================================
    ...
    try:
        ...
        for pc_lbl, sp_lbl, tid, d in tabs:
            if tid == "post" and d.empty:
                continue
            # data-pc と data-sp という属性に文字を預けるよ
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll(".tab-btn").forEach(button => {
        ...
        url = f"https://blog.hatena.ne.jp/{HATENA_USER}/{HATENA_BLOG}/atom/page/{HATENA_SCHEDULE_PAGE_ID}"
        xml = f'<?xml version="1.0" encoding="utf-8"?><entry xmlns="http://www.w3.org/2005/Atom"><title>2025 Game Schedule</title><content type="text/html">{escape(full_html)}</content></entry>'
        print("🚀 はてなブログを更新中...")
        res = requests.put(url, data=xml.encode("utf-8"), headers={"X-WSSE": wsse, "Content-Type": "application/xml"})
        if res.status_code == 200:
            print("✨ 成功したよ、しょう！")
        else:
            print(f"あちゃー: {res.status_code}")
